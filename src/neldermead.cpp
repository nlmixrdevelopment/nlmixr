...
typedef void (*fn_ptr) (double *, double *, double *, double *, int);
#define MXPAR  45
    ...
    *ynewlo = ylo;
    ...
    if (ystar < ylo) {
      if (*nfcall < kcount) {
        ...
        (*func)(p2star, &fval, yptr, fptr, len);
        y2star = fval;
        ++(*nfcall);
