...
if (identical(Sys.getenv("NLMIXR_VALIDATION"), "true")) {
  ...
  test_that("ODE", {
    ...
    expect_equal(signif(as.numeric(fit$logLik),6), -42649.6)
    expect_equal(signif(AIC(fit), 6), 85317.1)
    expect_equal(signif(BIC(fit), 6), 85378.8)  
    
    expect_equal(signif(as.numeric(fit$coefficients$fixed[1]),3), 6.89)
    expect_equal(signif(as.numeric(fit$coefficients$fixed[2]),3), 5.46)
    expect_equal(signif(as.numeric(fit$coefficients$fixed[3]),3), 4.22)
    expect_equal(signif(as.numeric(fit$coefficients$fixed[4]),3), -0.0236)
    
    expect_equal(signif(as.numeric(z[1, "StdDev"]), 3), 0.314)
    expect_equal(signif(as.numeric(z[2, "StdDev"]), 3), 0.000307)
