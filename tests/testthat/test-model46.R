...
context("NLME: two-compartment infusion, single-dose")
...
if (identical(Sys.getenv("NLMIXR_VALIDATION_FULL"), "true")) {
  ...
  test_that("Closed-form", {
    ...
    expect_equal(signif(as.numeric(fit$logLik),6), -11944.3)
    expect_equal(signif(AIC(fit), 6), 23906.6)
    expect_equal(signif(BIC(fit), 6), 23958.2)  
    ...
    expect_equal(signif(as.numeric(z[3, "StdDev"]), 3), 0.259)
  ...
  test_that("ODE", {
    ...
    expect_equal(signif(as.numeric(fitODE$logLik),6), -11943.7)
    expect_equal(signif(AIC(fitODE), 6), 23905.4)
    expect_equal(signif(BIC(fitODE), 6), 23957)  
    ...
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[4]),3), 3.9)
    
    expect_equal(signif(as.numeric(z[1, "StdDev"]), 3), 0.298)
    expect_equal(signif(as.numeric(z[2, "StdDev"]), 3), 0.303)
    expect_equal(signif(as.numeric(z[3, "StdDev"]), 3), 0.279)
    expect_equal(signif(as.numeric(z[4, "StdDev"]), 3), 0.276)
