...
if (identical(Sys.getenv("NLMIXR_VALIDATION_FULL"), "true")) {
  ...
  test_that("ODE", {
    ...
    expect_equal(signif(as.numeric(fitODE$logLik),6), -26997.9)
    expect_equal(signif(AIC(fitODE), 6), 54017.9)
    expect_equal(signif(BIC(fitODE), 6), 54088.8)  
    ...
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[3]),3), 1.45)
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[4]),3), 3.79)
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[5]),3), -0.0785)
    
    expect_equal(signif(as.numeric(z[1, "StdDev"]), 3), 0.317)
    expect_equal(signif(as.numeric(z[2, "StdDev"]), 3), 0.258)
    expect_equal(signif(as.numeric(z[3, "StdDev"]), 3), 0.391)
