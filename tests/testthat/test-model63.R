...
if (identical(Sys.getenv("NLMIXR_VALIDATION"), "true")) {
  ...
  test_that("ODE", {
    ...
    expect_equal(signif(as.numeric(fitODE$logLik),6), -38503.1)
    expect_equal(signif(AIC(fitODE), 6), 77028.1)
    expect_equal(signif(BIC(fitODE), 6), 77103.5)  
    ...
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[3]),3), 1.42)
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[4]),3), 3.86)
    expect_equal(signif(as.numeric(fitODE$coefficients$fixed[5]),3), -0.0848)
    
    expect_equal(signif(as.numeric(z[1, "StdDev"]), 3), 0.316)
    expect_equal(signif(as.numeric(z[2, "StdDev"]), 3), 0.261)
    expect_equal(signif(as.numeric(z[3, "StdDev"]), 3), 0.350)
    expect_equal(signif(as.numeric(z[4, "StdDev"]), 3), 0.282)
