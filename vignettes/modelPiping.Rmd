...
one.ka.0.5 <- fit %>%
    ini(tka=0.5) %>% 
    nlmixr(est="posthoc", table=list(cwres=TRUE, npde=TRUE))
...
one.ka.0.7 <- one.compartment %>%
    ini(tka=0.7) %>% 
    nlmixr(theo_sd, est="focei", table=list(cwres=TRUE, npde=TRUE))
...
addPropModel <- fit %>%
    ...
    ini(prop.err=0.1) %>%
    nlmixr(d,est="focei", table=list(cwres=TRUE, npde=TRUE))
