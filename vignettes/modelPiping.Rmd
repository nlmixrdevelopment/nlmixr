...
one.ka.0.5 <- fit %>%
    ini(tka=0.5) %>% # FIXME nlmixr should know I used theo_sd
    nlmixr(theo_sd, est="posthoc")
...
one.ka.0.5 <- fit %>%
    ini(tka=fix(0.5)) %>% # FIXME nlmixr should know I used theo_sd
    nlmixr(theo_sd, est="focei")
...
one.ka.0.5 <- fit %>%
    ini(tka=fix) %>% # FIXME nlmixr should know I used theo_sd
    nlmixr(theo_sd, est="focei")
...
one.ka.0.7 <- one.compartment %>%
    ini(tka=0.7) %>% # FIXME nlmixr should know I used theo_sd
    nlmixr(theo_sd, est="focei")
...
variabilities to certain model parameters.  For example, you could
