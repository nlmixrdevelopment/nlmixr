...
```{r}
fit <- nlmixr(uif, theo_md, est="nlme", calc.resid=FALSE)
```

We can express the same model by ordinary differential equations (ODEs):
...
```r
fit <- nlmixr(uif, theo_md, est="saem")
```
