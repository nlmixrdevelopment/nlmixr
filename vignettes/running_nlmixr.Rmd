...
```
fit <- nlmixr(one.cmt, theo_sd, "nlme")
print(fit)
#> 
...
#> elapsed 3.922 75.1171 0.053 1.105895
#> 
...
#> tka       Log Ka 0.447  0.192   43       1.56 (1.07, 2.28)     68.7     0.241% 
#> tcl       Log Cl  1.02 0.0847 8.31       2.77 (2.35, 3.27)     26.9      3.78% 
#> tv         log V  3.45 0.0464 1.35       31.5 (28.7, 34.5)     13.6      10.0% 
#> add.sd           0.697                               0.697                      
#> 
#>   Covariance Type (fit$covMethod): nlme
...
#> 1 1     0      0.74     0  0     0.74   0     0.74   1.06   0.101 -0.479 -0.0819
#> 2 1     0.25   2.84     0  3.25 -0.410  3.84 -1.00  -1.44   0.101 -0.479 -0.0819
#> 3 1     0.570  6.57     0  5.83  0.744  6.78 -0.212 -0.305  0.101 -0.479 -0.0819
#> # … with 129 more rows, and 6 more variables: rx1c <dbl>, ka <dbl>, cl <dbl>,
#> #   v <dbl>, depot <dbl>, central <dbl>
```
...
#> elapsed 18.142 31.54705 0.049      0.024 0.778949
#> 
...
#>           OBJF      AIC      BIC Log-likelihood Condition Number
#> FOCEi 116.8064 373.4062 393.5858      -179.7031         161.0359
#> 
#> ── Time (sec; fitF$time): ────────────────────────────────────────────────────── 
#>            setup optimize covariance table    other
#> elapsed 71.86674 0.321867   0.321868 0.059 1.797528
#> 
#> ── Population Parameters (fitF$parFixed or fitF$parFixedDf): ─────────────────── 
#>        Parameter  Est.     SE  %RSE Back-transformed(95%CI) BSV(CV%)
#> tka       Log Ka 0.469  0.229  48.8         1.6 (1.02, 2.5)     70.2
#> tcl       Log Cl  1.01   0.11  10.9       2.75 (2.21, 3.41)     26.8
#> tv         Log V  3.46 0.0248 0.716       31.8 (30.3, 33.4)     13.8
#> add.sd           0.694                                0.694         
#>        Shrink(SD)%
#> tka         1.60% 
#> tcl         3.94% 
...
#> add.sd             
#> 
#>   Covariance Type (fitF$covMethod): r,s
#>   No correlations in between subject variability (BSV) matrix
#>   Full BSV covariance (fitF$omega) or correlation (fitF$omegaR; diagonals=SDs) 
#>   Distribution stats (mean/skewness/kurtosis/p-value) available in fitF$shrink 
#>   Minimization message (fitF$message):  
#>     false convergence (8) 
...
#> 
#> ── Fit Data (object fitF is a modified tibble): ──────────────────────────────── 
#> # A tibble: 132 x 22
#>   ID     TIME    DV  EVID  PRED    RES   WRES IPRED   IRES  IWRES CPRED   CRES
#>   <fct> <dbl> <dbl> <int> <dbl>  <dbl>  <dbl> <dbl>  <dbl>  <dbl> <dbl>  <dbl>
#> 1 1     0      0.74     0  0     0.74   0.391  0     0.74   1.07   0     0.74 
#> 2 1     0.25   2.84     0  3.28 -0.436 -0.230  3.85 -1.01  -1.45   3.23 -0.393
#> 3 1     0.570  6.57     0  5.85  0.723  0.382  6.79 -0.216 -0.311  5.79  0.776
...
For diagnosing the model further, you can look at various methods for
retrieving model diagnostics including the included methods, xpose and
ggPMX.
